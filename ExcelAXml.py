...
# TODO Ultima modificación 21/07/2022
...
# El item se crea si el SID no existe. el service NO
# mostrar el caso en el que no hay numero pero si SID.
...
# ruta = str(input('Ingrese la ruta donde sera guardado el XML: '))
...
for fila in range(2, hoja.max_row+1):
    for columna in range(1, hoja.max_column+1):
        ...
        if(columna == 25):  # SAP ROUTER
            _SAProuter = str(cell_obj.value)
        if(columna == 27):  # MessageServer
            _MessageServer = str(cell_obj.value)
        if(columna == 28):  # Group Server
            _groupServer = str(cell_obj.value)
        if(columna == 29):  # Comentario
...
archivo.write(ruta + "SAPUILandscape.xml")
